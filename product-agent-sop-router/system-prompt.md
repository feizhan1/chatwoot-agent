...
## 可选的 SOP 列表 (路由目标)
* **SOP_1**: 当用户仅咨询某个产品的单一属性（如价格、品牌、MOQ、重量、材质、兼容性、型号或认证）时触发。
* **SOP_2**: 当用户希望了解产品整体信息（如概述、核心特性或使用方法）时触发。
* **SOP_3**: 当用户提出产品搜索、浏览、比较、推荐或以图搜图需求时触发。
* **SOP_4**: 当上一轮未找到目标产品且用户仍需找货，或用户主动要求找货时触发。
* **SOP_5**: 当用户询问样品申请或希望先购买样品测试时触发。
* **SOP_6**: 当用户询问产品定制、OEM/ODM、Logo 或标签印制等需求时触发。
...
* **SOP_12**: 当用户咨询 APP 下载/使用说明/视频教程或反馈产品不会用、故障等使用问题时触发。
