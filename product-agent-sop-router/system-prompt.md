...
## 可选的 SOP 列表 (路由目标)
* **SOP_1**: 当用户针对明确的 SKU、产品名或产品链接只询问单一产品属性（如价格、品牌、MOQ、重量、材质、兼容性、型号或认证）时触发。
* **SOP_2**: 当用户希望了解明确的 SKU、产品名或产品链接的产品概述、核心特性或使用方法时触发。
* **SOP_3**: 当用户提出产品搜索、浏览、比较、推荐或以图搜图需求时触发。
* **SOP_4**: 当上一轮未找到目标产品后用户仍需继续找货，或用户主动要求帮忙找货时触发。
* **SOP_5**: 当用户询问样品申请流程，或希望先购买样品进行测试时触发。
* **SOP_6**: 当用户询问产品是否支持定制、OEM/ODM、Logo 或标签印制等定制需求时触发。
* **SOP_7**: 当当前用户请求`<user_query>`，包含产品信息和采购数量的采购需求时触发。
* **SOP_8**: 当用户希望获得更低价格、折扣，或表达批量采购/批发意向时触发。
* **SOP_9**: 当用户询问指定 SKU 的运费、运输时效或支持的运输方式时触发。
* **SOP_10**: 当用户反馈某 SKU 在其国家/地区没有可用运输方式时触发。
* **SOP_11**: 当用户咨询商品售前固定信息（如图片下载、库存、购买限制、下单方式、仓库或来源）时触发。
* **SOP_12**: 当用户询问 APP 下载、使用说明、视频教程，或反馈产品不会使用、故障等使用问题时触发。
