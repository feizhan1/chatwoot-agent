...
**合并重复内容**：
- ❌ 避免：3个独立章节讲"简洁性"、"不啰嗦"、"语气约束"
- ✅ 正确：合并为1个"回复简洁性与准确性"章节

**流程图简化**：
- ❌ 避免：ASCII 流程图（占用10-20行）
- ✅ 正确：紧凑列表或简短代码块
...
用户请求订单修改（地址/取消/合并）
    ↓
【步骤 1 - 强制】调用 query-order-info-tool
    ↓
【步骤 2 - 判断】检查返回的订单支付状态
    ↓
    ├─ 订单状态 = 未支付 (Unpaid)
...
- 使用 ✅ 和 ❌ 标记正确/错误示例
- 使用 `**加粗**` 强调关键术语
- 使用 `行内代码` 标记工具名、字段名、枚举值

**代码块使用**：
- 流程图：使用三个反引号（无语言标签）
- JSON/代码示例：使用 ```json 或 ```javascript
- 引用模板：使用 `>` 块引用

**层级控制**：
- 一级标题 `#`：主要章节（角色、约束、目标、工具）
- 二级标题 `##`：子章节（具体约束、具体场景）
- 三级标题 `###`：细分（场景的子类）
- **最多使用三级标题**，避免过深嵌套
...
**目标行数**（system-prompt.md）：
- 简单 agent（无工具调用）：100-150 行
- 中等 agent（2-3 个工具）：200-300 行
- 复杂 agent（4+ 工具）：300-400 行
...
**检查清单**：
- [ ] 是否有3个以上章节在讲同一件事？→ 合并
- [ ] 是否有超过30行的 ASCII 流程图？→ 简化为列表
- [ ] 是否有超过5个示例的规则？→ 减少到1-2个
- [ ] 是否有10个以上独立场景执行相同操作？→ 合并为列表
- [ ] 是否有超过20行的单一段落？→ 分段或改为列表

#### 6. 实际案例对比

**精简前**（order-agent，641 行）：
- 3个独立章节讲"简洁性"（回复简洁性、禁止回避、语气约束）
- 30行 ASCII 流程图
- 9个独立"转人工"场景（每个5-10行）
- 每个规则5-6个示例
...
**可追溯性**：
- 关键规则应有明确标题，便于引用
- 使用 `（见"无法准确回复时的处理"章节）` 进行内部引用

**版本友好**：
- 新增规则应添加到合适章节，而非新建独立章节
- 删除过时规则时，检查是否有其他章节引用
